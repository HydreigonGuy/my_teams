...
void fill_thread_resp_data(char *data, thread_t *thread)
{
    int c = 0;
    ...
    data[c] = '\a';
    c++;
    fill_thread_resp_time(data, thread, c);
}
...
void send_thread_info(connection_t *con, thread_t *thread)
{
    ...
    if (!data)
        return;
    fill_thread_resp_data(data, thread);
    create_response(con->write_buff, 393, data, "Sent thread info");
    free(data);
}
