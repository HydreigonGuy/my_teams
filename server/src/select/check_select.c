...
connection_t *connect)
{
    for (connection_t *curr = connect; curr; curr = curr->next) {
        if (curr->status == READ && FD_ISSET(curr->fd, r_fds))
            curr->ready = 1;
        if (curr->status == WRITE && FD_ISSET(curr->fd, w_fds))
            curr->ready = 1;
        if (curr->status == EXCEPT && FD_ISSET(curr->fd, ex_fds))
            curr->ready = 1;
    }
}

void check_select(my_server_t *server, connection_t *connect)
{
    struct timeval tv;
    fd_set read_fds;
    fd_set write_fds;
